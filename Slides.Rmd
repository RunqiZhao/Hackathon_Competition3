...
tabsetPanel(tabPanel(HTML("<span style='font-size:18px'>Top Skills</span>")
                     , column(6, dataTableOutput("skilltable"))
                     , column(6, plotOutput("skillplot"))
                     ),
            tabPanel(h6("Top Certificates")
                     , column(6, dataTableOutput("certitable"))
                     , column(6, plotOutput("certiplot"))
                     )
            ) #end of tabsetPanel
...
output$skillplot <- renderPlot({
  ggplot(data = subset(job_skill[1:round(length(job_skill$State)*0.002),],State == input$states,), mapping=aes(n,reorder(Skill,n), fill=-n)) +
  geom_col()+
  xlab("count")+
  ylab("skill")+
